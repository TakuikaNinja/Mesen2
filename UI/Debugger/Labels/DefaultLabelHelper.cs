	...
	public class DefaultLabelHelper
	{
		...
		private static void SetDefaultNesLabels()
		{
			...
			if(EmuApi.GetRomInfo().Format == RomFormat.Fds) {
				...
				LabelManager.SetLabel(0x086A, MemoryType.NesPrgRom, "WriteVRAMBuffer", "Input: " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A, X, Y, $0301, $0302 " + Environment.NewLine + "Desc: Write the VRAM Buffer at $0302 to VRAM. Refer to NESdev Wiki for information on the structure.");
				LabelManager.SetLabel(0x08B3, MemoryType.NesPrgRom, "ReadVRAMBuffer", "Input: X = Start address of read buffer, Y = # of bytes to read from VRAM " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A, X, Y, $0300-$03ff " + Environment.NewLine + "Desc: Read individual bytes from VRAM to the VRAM buffer. (see notes on NESdev Wiki)");
				LabelManager.SetLabel(0x08D2, MemoryType.NesPrgRom, "PrepareVRAMString", "Input: A = High VRAM address, X = Low VRAM address, Y = string length, Direct Pointer = data to be written to VRAM " + Environment.NewLine + "Output: A = $ff : success, A = $01 : string didn't fit in buffer " + Environment.NewLine + "Affects: A, X, Y, $00-$06, $0301, $0302-$03ff " + Environment.NewLine + "Desc: Copy pointed data into the VRAM buffer at $0302. Data which does not fit in the buffer (according to the max index variable defined in $0300) is rejected. (see notes on NESdev Wiki)");
				LabelManager.SetLabel(0x08E1, MemoryType.NesPrgRom, "PrepareVRAMStrings", "Input: A = High VRAM address, X = Low VRAM address, Direct pointer = data to be written to VRAM " + Environment.NewLine + "Output: A = $ff : success, A = $01 : data didn't fit in buffer " + Environment.NewLine + "Affects: A, X, Y, $00-$06, $0301, $0302-$03ff " + Environment.NewLine + "Desc: Copy a 2D string into the VRAM buffer at $0302 as multiple 1D strings. The first byte of the data determines the width and height of the 2D string (in tiles): Upper nybble = height, lower nybble = width. Data which does not fit in the buffer (according to the max index variable defined in $0300) is rejected. (see notes on NESdev Wiki)");
				LabelManager.SetLabel(0x094F, MemoryType.NesPrgRom, "GetVRAMBufferByte", "Input: X = starting index of read buffer, Y = VRAM read structure to check (starting at 1), $00 = High VRAM address, $01 = Low VRAM address " + Environment.NewLine + "Output: carry clear : a previously read byte was returned in A, carry set : no byte was read, program must call ReadVRAMBuffer " + Environment.NewLine + "Affects: A, X, Y, $0300-$03ff " + Environment.NewLine + "Desc: This routine was likely intended for avoiding repeated VRAM reads (see notes on NESdev Wiki). It compares the target VRAM address in ($00) with the Yth structure in the read buffer ($0300+X+(Y-1)*3). If the addresses match, the carry is cleared and the corresponding data byte is returned in A. Otherwise, the target VRAM address is overwritten with the contents of ($00) and the routine exits with the carry set to indicate that the program must call ReadVRAMBuffer to obtain the desired data. ");
				...
				LabelManager.SetLabel(0x09B1, MemoryType.NesPrgRom, "Random", "Input: X = Zero Page address where the random bytes are placed, Y = # of shift register bytes (normally $02) " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A, X, Y, $00 " + Environment.NewLine + "Desc: This is a shift-register based pseudo-random number generator which normally takes 2 bytes. Programs should seed the bytes with nonzero values before use. Each call of this routine will shift the bytes right by one bit.");
				...
				LabelManager.SetLabel(0x09D3, MemoryType.NesPrgRom, "CounterLogic", "Input: A, Y = end zero-page address of counters, X = start zero-page address of counters " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A, X, $00 " + Environment.NewLine + "Desc: Decrement several counters in zero-page. The first counter is a decimal counter 9 -> 8 -> 7 -> ... -> 1 -> 0 -> 9 -> ... Counters 1...A are simply decremented and stay at 0. Counters A+1...Y are decremented when the first counter does a 0 -> 9 transition, and stay at 0.");
				LabelManager.SetLabel(0x09EB, MemoryType.NesPrgRom, "ReadPads", "Input: " + Environment.NewLine + "Output: $f5 = Joypad #1 data, $f6 = Joypad #2 data, $00 = Expansion #1 data, $01 = Expansion #2 data " + Environment.NewLine + "Affects: A, X " + Environment.NewLine + "Desc: Read hardwired/expansion port joypads. Expansion port reads should be placed/used elsewhere if needed after calling this routine, otherwise they will be clobbered by other BIOS calls.");
				...
				LabelManager.SetLabel(0x0AEA, MemoryType.NesPrgRom, "SetScroll", "Input: " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A " + Environment.NewLine + "Desc: Set the scroll registers according to values in $fc, $fd and $ff. Should typically be called in vblank after VRAM updates.");
				...
				LabelManager.SetLabel(0x0B13, MemoryType.NesPrgRom, "ReadKeyboard", "Input: " + Environment.NewLine + "Output: A = $ff : success, A = $00 : no keyboard connected, $00-$08 = keyboard data" + Environment.NewLine + "Affects: A, X, Y, $00-$08, $fb " + Environment.NewLine + "Desc: This attempts to read the Family BASIC Keyboard, returning with A=0 if nothing is connected. Keyboard data is stored at $00-$08 starting from row 8 in descending order ($00 = row 8, $01 = row 7, and so on), with each byte containing column 0's data in the lower nybble and column 1's data in the upper nybble. A set bit means that a key was pressed.");
				LabelManager.SetLabel(0x0B66, MemoryType.NesPrgRom, "LoadTileset", "Input: A = Low VRAM Address & Flags, Y = Hi VRAM Address, X = # of tiles to transfer to/from VRAM " + Environment.NewLine + "Output: " + Environment.NewLine + "Affects: A, X, Y, $00-$04 " + Environment.NewLine + "Desc: Read/write 2BP and 1BP tilesets to/from VRAM. See appendix on NESdev Wiki regarding the flags.");
